    ...
    if type(firstStep) == tuple:
        ...
        if type(location) == ast.Slice:
            ...
            if childType == "vararg":
                ...
                log(
                    "generate_message\tgetLineNumber\tMissing childType for arguments: "
                    + str(childType),
                    "bug",
                )
...
def getColumnNumber(tree, path, value):
    ...
    if type(firstStep) == tuple:
        ...
        if type(location) == ast.Slice:
            ...
            if childType == "arg":
                return location.value.col_offset - len(" : ") - len(location.arg)
        elif type(location) == ast.alias:
            if childType == "name":
                return getColumnNumber(tree, path[1:], None)
            elif childType == "asname":
                return (
                    getColumnNumber(tree, path[1:], None) + len(tree.name) + len(" as ")
                )
        ...
        if childType in ["id", "n", "s"]:
            ...
            if type(location) == ast.FunctionDef:
                return location.col_offset + len("def ") + len(location.name) + len("(")
            elif type(location) == ast.Lambda:
        ...
        if firstStep == -1:
            ...
            if 0 < len(l) <= firstStep:
                if type(location) == ast.Dict:
                    if childType == "values" and len(location.keys) > len(
                        location.values
                    ):
...
def reduceToOneToken(oldVal, newVal, typeVector):
    keepFirst = type(oldVal) == type(
        newVal
    )  # if the type matched, keep the first field choice
    ...
    if type(newVal) in [
        ...
        ast.Interactive,
        ast.Suite,
    ]:  # adding a function or class item
        ...
        if keepFirst:
            if len(newVal.body) > 0:
                ...
                tmpDecorators = [
                    reduceToOneToken(
                        getFirst(oldVal.decorator_list), newVal.decorator_list[0], None
                    )
                ]
        elif isSubvector:
            for i in range(len(newVal.body)):
        ...
        if keepFirst:
            if len(newVal.targets) > 0:
                tmpTargets = [
                    reduceToOneToken(getFirst(oldVal.targets), newVal.targets[0], None)
                ]
        elif isSubvector:
        ...
        tmpItems = (
            [reduceToOneToken(None, newVal.items[0], None)]
        ...
        if keepFirst:
            if len(newVal.items) > 0:
                tmpItems = [
                    reduceToOneToken(getFirst(oldVal.items), newVal.items[0], None)
                ]
        ...
        tmpBody = formatStatementList(newVal.body, "~body~")
        tmpHandlers = (
            [reduceToOneToken(None, newVal.handlers[0], None)]
            if len(newVal.handlers) > 0
            else []
        )
        tmpOrelse = formatStatementList(newVal.orelse, "~else body~")
        ...
        if keepFirst:
            if len(newVal.body) > 0:
                ...
                tmpOrelse = [
                    reduceToOneToken(getFirst(oldVal.orelse), newVal.orelse[0], None)
                ]
            elif len(newVal.finalbody) > 0:
                tmpFinalbody = [
                    reduceToOneToken(
                        getFirst(oldVal.finalbody), newVal.finalbody[0], None
                    )
                ]
        elif isSubvector:
            for i in range(len(newVal.body)):
        ...
        newVal.value = reduceToOneToken(
            oldVal.value if type(oldVal) == ast.Expr else None,
            newVal.value,
            "sub" if isSubvector else None,
        )
    elif type(newVal) == ast.BoolOp:
        # Leave the op alone
        tmpValues = (
            [ast.Str("~left value~"), ast.Str("~right value~")]
        ...
        if keepFirst:
            if len(newVal.values) > 0:
                tmpValues = [
                    reduceToOneToken(getFirst(oldVal.values), newVal.values[0], None),
                    ast.Str("~right value~"),
                ]
        elif isSubvector:
        ...
        if keepFirst:
            if len(tmpKeys) > 0:
                tmpKeys = [
                    reduceToOneToken(getFirst(oldVal.keys), newVal.keys[0], None)
                ]
        ...
        if keepFirst:
            if len(tmpElts) > 0:
                tmpElts = [
                    reduceToOneToken(getFirst(oldVal.elts), newVal.elts[0], None)
                ]
        elif isSubvector:
        ...
        tmpGenerators = (
            [reduceToOneToken(None, newVal.generators[0], None)]
            if len(newVal.generators) > 0
            else []
        )
        if keepFirst:
                ...
                for i in range(len(newVal.args)):
                    if occursIn(oldVal, newVal.args[i]):
                        if len(newVal.args) == 1:
                            ...
                            tmpArgs = [
                                ast.Str("~starting args~"),
                                oldVal,
                                ast.Str("~remaining args~"),
                            ]
                        break
                else:
        ...
        tmpDims = (
            [reduceToOneToken(None, newVal.dims[0], None)]
        ...
        if keepFirst:
            if len(newVal.dims) > 0:
                tmpDims = [
                    reduceToOneToken(getFirst(oldVal.dims), newVal.dims[0], None)
                ]
        elif isSubvector:
        ...
        log(
            "generate_message\treduceToOneToken\tMissing AST type: "
            + str(type(newVal)),
            "bug",
        )
    return newVal
    ...
    while i < len(edit):
        ...
        if line == -1 or col == -1:
            if len(cv.path) == 2 and cv.path[0] == 0:
                ...
                log(
                    "generate_message\tformatHints\tCouldn't find line/col number: "
                    + "IDWOULDBEHERE"
                    + ";"
                    + str(cv)
                    + "\n"
