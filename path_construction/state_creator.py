...
def applyChangeVectors(s, changes) -> State:
...
def optimizeGoal(s: CodeState, changes):
	...
	currentGoal, currentDiff, currentEdits = s.goal, s.distance_to_goal, changes # set up values that will change
	allChanges = []
	...
	while len(treeLevel) != 0:
		...
		for branch in treeLevel:
			...
			for i in range(len(branch.next)):
				...
				newState = applyChangeVectors(s, newChanges)
	...
	if s.goal.tree == currentGoal.tree:
		return allChanges # optimize so we don't need to do the power set twice
	else:
		s.goal, s.distance_to_goal = currentGoal, currentDiff # otherwise, put in the new goal
