            ...
            if location in mapDict["pos"]:
                ...
                if location < mapDict["len"]:
                    ...
                    if i >= 0:
                        ...
                        log("AddVector\tupdate\t\tMissing position: " + str(location) + "," + str(mapDict["pos"]),
                            "bug")
                        return
...
class SwapVector(ChangeVector):
    ...
    def applyChange(self, caller=None):
        ...
        if self.oldPath == None:
            ...
            if type(treeSpot) == list and self.oldSubtree < len(treeSpot) and \
                ...
                (treeSpot[self.oldSubtree], treeSpot[self.newSubtree]) = (
                    treeSpot[self.newSubtree], treeSpot[self.oldSubtree])
            else:
    ...
    def update(self, newStart, mapDict):
        ...
        if self.oldPath == None:
            ...
            (mapDict["pos"][self.oldSubtree], mapDict["pos"][self.newSubtree]) = (
                mapDict["pos"][self.newSubtree], mapDict["pos"][self.oldSubtree])
        else:
            ...
            if type(oldTreeSpot) == type(newTreeSpot) == int:
                (oldMapDict["pos"][self.oldSubtree], newMapDict["pos"][self.newSubtree]) = (
                    newMapDict["pos"][self.newSubtree], oldMapDict["pos"][self.oldSubtree])
...
class MoveVector(ChangeVector):
    ...
    def update(self, newStart, mapDict):
        ...
        if self.oldSubtree not in mapDict["pos"]:
            log("MoveVector\tupdate\t\tCan't find old subtree: " + str(self.oldSubtree) + "," + str(mapDict["pos"]),
                "bug")
            return
